...
if(isset($_POST['Submit'])) {	
	...
	$total = mysqli_real_escape_string($con, $_POST['total']);
		...
		//$tempname = base64_encode(file_get_contents(addslashes($tempname)));
		...
		$adharname = $_FILES["uploadadhar"]["name"];
		$adhartempname = $_FILES["uploadadhar"]["tmp_name"];
	...
	if(empty($name) && empty($cardno) && empty($filename) && empty($age) && empty($village) && empty($adharname) && empty($appname) && empty($formname) && empty($small) && empty($big) && empty($cow) && empty($bull) && empty($calf) && empty($total)) {
		...
		$result = mysqli_query($con, "INSERT INTO users(name,cardno,image,age,village,adhar,application,satbara,small,big,cow,bull,calf,total) VALUES('$name','$cardno','$folder','$age','$village','$adharfolder','$appfolder','$formfolder','$small','$big','$cow','$bull','$calf','$total')");
